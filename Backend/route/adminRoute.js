...
import { propertyModel, buyerModel, userModel, contactUsModel } from '../model/table.js'
...
adminRoute.post('/add-property', async (req, res) => {
    try {
        ...
        if (isExist) {
            ...
            const data = new propertyModel({ title, price, area, description, location, pic: picName })
...
adminRoute.get('/admin-contact-list', async (req, res) => {
    try {
